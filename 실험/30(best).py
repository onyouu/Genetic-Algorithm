...
while run:
    ...
    if start-end > interval:
        ...
        if len(all_creatures)<max_num:
            ...
            for i in black_genes:
                i[2] += 1

            # ACG 계산 추가
            if black_genes:
                current_acg = sum(x[2] for x in black_genes) / len(black_genes)
                acg_list.append(current_acg)
...
with open("acg_data.csv", "w", newline='') as file:
    writer = csv.writer(file)
    writer.writerow(["Generation", "ACG"])
    for gen, acg in enumerate(acg_list):
        writer.writerow([gen, acg])

# ACG 그래프 그리기
plt.plot(acg_list, color='blue')
plt.title("Average Continuous Generation per Generation")
