# 포식자:빨간색, 도망자:검정색 - 로트카-볼테라 방정식 적용 시켜도 괜찮을 듯
# 유전자 파리미터(크기, 속도)
# 유전 알고리즘 적용 해야하는데 유전자 형태 어케 만들어야 하냐ㅠㅠ - 구현완료
# 처음 초기 도망자 개체에 색깔 부여해서 그 자식들 살짝씩 색 변형해서 구별하기 쉽게 하기 - 구현완료
# 엄마의 색이랑 아빠의 색 섞은 거? - 구현완료
# 아니 거기다가 균등교차니까 마스크 값으로 섞어서 자식 색 만들어도 개 쩔듯 - 구현완료
# 아들 개체 생성할 때 부모의 개체의 위치나 그 근처에서 생성되게 하기 - 구현완료
# 실험 계획
# 변이율을 여러 단계로 나눠서 실험 진행
# 변이율에 따른 평균 지속세대 확인(100세대가 되었을 때) - 목표 도달도 확인
# 그 후에 다시 평균 지속세대 확인(500세대가 되었을 때) - 지속도 확인
# 그 외에 다양한 변수들 확인(가로크기,세로크기,속도,활동성)
# 0.5 이하는 활동성이 가장 빨리 바뀌면서 안전구역에 빨리 모임
# 0.6은 크기가 가장 빨리 바뀌면서 전체적으로 분포
